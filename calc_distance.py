...
JSON_PATH = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'geoinfo-all.json'
DB_PATH = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'geoinfo-all.db'

def save_db(all_city_list):
    """ save [(city_name,(lat,log),level),..] into db """
    cx = sqlite3.connect(DB_PATH)
    cu = cx.cursor()
    cu.execute("create table if not exists geo_info (district varchar(50), city varchar(50), prov varchar(50), lat real, log real, LEVEL integer)")
...
def query_db(loc_name):
    ...
    cf = cu.fetchall()
    if len(cf) <> 1:
        print u'[%s]查询结果不唯一, 请输入更准确的地名' % loc_name
        for city in cf:
...
def main(argv):
    ...
    if len(argv)<>3:
        city_a = raw_input('input city 1:').decode(sys.stdin.encoding)
        city_b = raw_input('input city 2:').decode(sys.stdin.encoding)
    else:
        city_a = unicode(argv[1],'gb2312')
        city_b = unicode(argv[2],'gb2312')
    ...
    if city_a and city_b:
        ...
        print u"从[%s]到[%s]的距离是: %d km" % (
            city_a[2]+'-'+city_a[1]+'-'+city_a[0], 
            city_b[2]+'-'+city_b[1]+'-'+city_b[0], 
            int(vincenty(loc_a, loc_b).km))
